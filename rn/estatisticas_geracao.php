...
try {
  ...
  if ($objEstatisticasDTORet != null){
    ...
    if($bolAcaoImprimir){
      ...
      //$arrComandos[] = '<button type="button" accesskey="I" id="btnImprimir" value="Imprimir" onclick="printDiv(\'divTabelas\');" class="infraButton"><span class="infraTeclaAtalho">I</span>mprimir</button>';
    }
	    	...
	    	foreach ($arrUnidades as $keyUnidade => $arrTipos){
		      ...
		      if ($arrObjUnidadeDTO[$keyUnidade]!=null){
	    		...
	    		foreach ($arrTipos as $keyTipo => $arrMeses){
						foreach ($arrMeses as $keyAno => $arrMeses) {
							foreach ($arrMeses as $keyMes => $quantidade){
								$total += $quantidade;
							}
						}
	    		}	
    ...
    //stevenes
	 //$totalTabelaGerados = $total;
   $totalTabelaGerados = array();
  	$totalTabelaGerados[0] = 'Processos Gerados na Unidade';      
  	$totalTabelaGerados[1] = InfraUtil::formatarMilhares($total);      
  	$totalTabelaGerados[2] = $total;      
   $totalTabelaGerados[3] = "#"; 	
    	...
    	foreach ($arrUnidades as $keyUnidade => $arrTipos){
	      ...
	      if ($arrObjUnidadeDTO[$keyUnidade]!=null){
    		...
    		foreach ($arrTipos as $keyTipo => $quantidade){
		      ...
		      //stevenes
		      //$totalTabelaTramitacao = $total;
		      $totalTabelaTramitacao = array();
	  		   $totalTabelaTramitacao[0] = 'Processos com tramitacao no período';      
	  		   $totalTabelaTramitacao[1] = InfraUtil::formatarMilhares($total);      
	  		   $totalTabelaTramitacao[2] = $total;      
	  		   $totalTabelaTramitacao[3] = "#"; 		      		      		      
	      ...
	      if ($arrObjUnidadeDTO[$keyUnidade]!=null){
	    	...
	    	foreach ($arrTipos as $keyTipo => $quantidade){
		      ...
		      $contadorTabelaFechados++;
		       ...
		       //stevenes
		      //$totalTabelaFechados = $total;
		      $totalTabelaFechados = array();
	  		   $totalTabelaFechados[0] = 'Processos com andamento fechado na unidade';      
	  		   $totalTabelaFechados[1] = InfraUtil::formatarMilhares($total);   
	  		   $totalTabelaFechados[2] =  $total;     
	  		   $totalTabelaFechados[3] = "#"; 		      		      
    		}
    	}
    }	
    	...
    	foreach ($arrUnidades as $keyUnidade => $arrTipos){
	      ...
	      if ($arrObjUnidadeDTO[$keyUnidade]!=null){
	        ...
	        $strResultadoTabelaAbertos .= '<td align="left">'.PaginaSEI::tratarHTML($strNomeTipoProcedimento).'</td>';
		      ...
		      //stevenes
		      //$totalTabelaAbertos = $total;	
		      $totalTabelaAbertos = array();
	  		   $totalTabelaAbertos[0] = 'Processos com andamento aberto';      
	  		   $totalTabelaAbertos[1] = InfraUtil::formatarMilhares($total);      
	  		   $totalTabelaAbertos[2] = $total;      
	  		   $totalTabelaAbertos[3] = "#"; 	      
    		}
    	}
    }	
		$strResultadoAbertos = '';
    ...
    if (!$bolOuvidoria){
   	 ...
   	 //stevenes
       $totalTabelaDocumentoGerado = array();
  		 $totalTabelaDocumentoGerado[0] = 'Documentos Gerados';      
  		 $totalTabelaDocumentoGerado[1] = InfraUtil::formatarMilhares($total);      
  		 $totalTabelaDocumentoGerado[2] = $total;      
  		 $totalTabelaDocumentoGerado[3] = "#";     
    ...
    if (!$bolOuvidoria){
      ...
      $strResultadoDocumentoRecebido .= '</table>';
        ...
        //stevenes
       $totalTabelaDocumentoRecebido = array();
  		 $totalTabelaDocumentoRecebido[0] = 'Documentos Externos';      
  		 $totalTabelaDocumentoRecebido[1] = InfraUtil::formatarMilhares($total);      
  		 $totalTabelaDocumentoRecebido[2] = $total;      
  		 $totalTabelaDocumentoRecebido[3] = "#";      
  		 
  		 //InfraUtil::formatarMilhares($quantidade), $quantidade, $strLink
    }
...
<form id="frmEstatisticas" onsubmit="return validarFormulario();" method="post" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  ?>
		...
		<label id="lblOrgao" for="selOrgao" accesskey="" class="infraLabel">Órgão:</label>
...
//stevenes - grafico
	...
	if(!$totalTabelaFechados) {

		$total = 0;
      $totalTabelaFechados = array();
	   $totalTabelaFechados[0] = 'Processos com andamento fechado ';      
  	   $totalTabelaFechados[1] = InfraUtil::formatarMilhares($total);   
  	   $totalTabelaFechados[2] =  $total;     
  	   $totalTabelaFechados[3] = "#"; 		
	} 
		
	//$arrGraficoGeralTotal = array();
	$arrGraficoGeral = array();
	$arrGraficoGeral[0] = $totalTabelaGerados;
	$arrGraficoGeral[1] = $totalTabelaTramitacao;
	$arrGraficoGeral[2] = $totalTabelaFechados;
	$arrGraficoGeral[3] = $totalTabelaAbertos;
	$arrGraficoGeral[4] = $totalTabelaDocumentoGerado;
	$arrGraficoGeral[5] = $totalTabelaDocumentoRecebido;
	
	$arrGraficoGeralTotal[$strSiglaUnidade.' / '.$strSiglaOrgao][] = $arrGraficoGeral;
...
foreach ($arrGraficoGeral as $fila) {	
  	$arrData[] = $fila[0];
   $arrAcesso[] = $fila[1];
} 

$nomes = implode("','",$arrData);
$quantidade = implode(",",$arrAcesso);

if ($contadorTabelaGeradosTotal > 0 || $contadorTabelaGerados > 0 || $contadorTabelaTramitacao > 0 || $contadorTabelaFechados > 0 || $contadorTabelaAbertos > 0 || $contadorTabelaDocumentoGerado > 0  ) {

echo	'<div id="chart" style="width:90%; float: left; margin-bottom: 10px; margin-top:30px;">
			<div id="unidades" style="width:98%; height:400;"></div>
		</div>';
}	
...
$(function () {
  $('#unidades').highcharts({
	 ...
	 credits: {
      ...
      title: {
          text: 'ESTATÍSTICAS DA UNIDADE:  <?php echo $strSiglaUnidade.' / '.$strSiglaOrgao; ?>'
      },
     
      xAxis: {
          categories: [ <?php  echo "'".$nomes."'"; ?> ], 
          labels: { 
          	  text: '',	                    
              align: 'center',
              style: {
